...
{
    const [ inventory, setInventory ] = useState(new Array(30).fill(0).map((_, i) =>
    {
        return {
            number: i + 1,
    ...
    return <div className="h-full gap-x-12 flex px-8 py-12">
        <div className="w-2/3 inventory-left-in">
            ...
            <p className="w-2/3 text-neutral-300 text-sm mt-2">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Nam quaerat dolore veniam maxime laudantium
                modi quos debitis commodi architecto inventore distinctio esse itaque nostrum tempora, deserunt, sed
        ...
        <div className="w-1/3 text-white inventory-right-in">
            {active && <div className="sticky top-5 flex flex-col">
                ...
                <p className="mt-2 text-sm text-neutral-400">
                    Lorem ipsum dolor sit amet consectetur adipisicing elit. Rerum quam deserunt, soluta delectus
                    veritatis impedit animi. Ad maiores veritatis, quo, laudantium beatae et minus quaerat aperiam
                    ipsum numquam fuga reprehenderit?
                </p>

                {!active.assign && <button
                    onClick={() => assign()}
                    className="bg-violet-500 px-8 py-3 rounded-full ml-auto mt-24"
                >
                    Assign
                </button>}
                {active.assign && <button onClick={() => unassign()} className="bg-violet-500 px-8 py-3 rounded-full ml-auto mt-24">
                    Unassign
                </button>}
            </div >}
        </div >
